  ...
  <div class="px-4 py-2 flex flex-col gap-2 mt-2" v-if="currentCommentBox === pid">
    ...
    <div v-if="userShow" class="flex gap-1">
      <template v-if="!global.userinfo.token">
        <UInput placeholder="姓名" v-model="state.username" autocomplete="name"/>
        <UInput placeholder="网站" v-model="state.website" autocomplete="url"/>
        <UInput placeholder="邮箱" v-model="state.email" autocomplete="email"/>
      </template>
    </div>
  </div>
</template>
