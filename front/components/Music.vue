...
const props = withDefaults(defineProps<MusicDTO>(), { mode: 'platform', id: '', url: '', name: '', lrc: '', artist: 'Audio artist', server: 'netease', type: 'song', api: defaultApi })
...
const id = ref(props.id || ''), url = ref(props.url || ''), name = ref(props.name || ''), lrc = ref(props.lrc || ''), artist = ref(props.artist || 'Audio artist')
...
const draft = computed<MusicDTO>(() => mode.value === 'direct' ? { mode:'direct', url:url.value.trim(), name:name.value.trim(), artist:artist.value.trim() || 'Audio artist', lrc:lrc.value } : { mode:'platform', id:id.value.trim(), server:server.value, type:type.value, api:api.value.trim() })
...
const reset = (close: Function) => { previewing.value=false;mode.value='platform';id.value='';url.value='';name.value='';lrc.value='';artist.value='Audio artist';server.value='netease';type.value='song';api.value=defaultApi;emit('confirm',{});close() }
watch(() => props, value => { mode.value=value.mode || (value.url?'direct':'platform');id.value=value.id||'';url.value=value.url||'';name.value=value.name||'';lrc.value=value.lrc||'';artist.value=value.artist||'Audio artist';server.value=value.server;type.value=value.type;api.value=value.api||defaultApi }, {deep:true})
</script>
