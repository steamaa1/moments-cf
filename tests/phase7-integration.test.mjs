...
assert.match(xPreview, /IntersectionObserver/);
assert.match(xPreview, /X 原帖加载失败/);
assert.match(xPreview, /i-carbon-renew/);
assert.match(xPreview, /animate-spin/);
assert.match(xPreview, /platform\.twitter\.com\/widgets\.js/);
assert.match(xPreview, /widgets!\.createTweet/);
assert.match(xPreview, /rootMargin: '300px 0px'/);
assert.match(xPreview, /align: 'center'/);
assert.doesNotMatch(xPreview, /<iframe|Nitter|nitter|CF-IPCountry|sessionStorage|x-embed-frame/);
assert.doesNotMatch(xPreview, /在 X 中查看|X 原帖<\/span>|weibo-card|x-card__/);
assert.match(memoEdit, /updateX/);
